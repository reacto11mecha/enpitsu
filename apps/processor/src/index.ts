...
const tracer = trace.getTracer("question-processor-tracer");
...
const worker = new Worker<QueueValue>(
  QUEUE_KEY,
  async (job) => {
    return tracer.startActiveSpan(`process-question-job`, async (span) => {
      ...
      try {
        ...
        logger.error({ err: error, jobId: job.id }, "[WORKER] Job failed");
        span.recordException(error as Error); // Capture error in SigNoz trace
        throw error;
      } finally {
        span.end();
      }
...
worker.on("failed", (job, err) => {
  logger.error(
    { err, jobId: job?.id },
    `[WORKER] Job failed with error: ${err.message}`,
  );
});

worker.on("error", (err) => {
  logger.error({ err }, `[WORKER] Internal BullMQ error: ${err.message}`);
});
...
void (async () => {
  try {
    ...
    logger.error(
      { err: error },
      "[WORKER] Failed to start due to a fatal error",
    );
  }
})();
